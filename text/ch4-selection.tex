...
\subsubsection{\boldmath \decay{\Lb}{\Lc\Kstar} background in \kk mass spectrum}
\label{sec:massfit:Lb2LcKst}

The mass fit model described in this section is used to model all \Dz modes, however, there is an additional source of background included in the \kk model that does not occur in the other modes. This additional background in the \kk \Bm mass spectrum comes from the decay \decay{\Lb}{\Lc(p\Km\pip)\Kstarm}, where the proton is misidentified as a kaon and the \pion is missed in the reconstruction. This background only contributes to the \kk mass spectrum as the combination of the misidentification of the proton and the pion being missed in the reconstruction results in a reconstructed \Bm mass distribution in the mass fit region. The decay mode \decay{\Lc}{p\Km\pip} accounts for over 6\% of the \Lc branching fraction~\cite{PDG2016}, whereas \Lc decays that may contribute as background to the other \Dz decay modes, e.g. \decay{\Lc}{p\pim\pip}, are suppressed by an order of magnitude. Therefore, this background is only considered for the \kk decay model.
