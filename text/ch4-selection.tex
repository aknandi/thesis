...
\begin{equation}
P_{sig}(m) = \mathcal{N}_{sig} . DCB(m| \mu,\sigma,\alpha,n,f_{\sigma}) \text{	where, } \int_{m_0}^{m_1} P_{sig}(m)\ dm = 1 \text{ , }
\end{equation}
...
The total partially reconstructed PDF, $P_{partreco}$, normalised to unity, is given by,
\begin{equation}
...
\begin{align*}
P_0 &= \frac{1}{1 + c^{Bu,\gamma}_0 + c^{Bd,\pi}_0} \left( P^{Bu,\pi}_0 + c^{Bu,\gamma}_0P^{Bu,\gamma}_0 + c^{Bd,\pi}_0P^{Bd,\pi}_0 \right) \\
P_{\pm 1} &= \frac{1}{1 + c^{Bu,\gamma}_{\pm 1} + c^{Bd,\pi}_{\pm 1}} \left( P^{Bu,\pi}_{\pm 1} + c^{Bu,\gamma}_{\pm 1}P^{Bu,\gamma}_{\pm 1} + c^{Bd,\pi}_{\pm 1}P^{Bd,\pi}_{\pm 1} \right) \text{ ,}
\end{align*}
where $P_x^{y,z}$ represents a partially reconstructed PDF, where $x$ is the \Dstar helicity state ($0$ or $\pm 1$), $y$ is the type of \B meson ($Bu$ or $Bd$) and $z$ is the particle missed in the reconstruction (\pion or \Pgamma). Similarly, $c_x^{y,z}$ are the ratios between the yields. The yield ratios between \decay{\Bm}{(\decay{\Dstarz}{\Dz[\piz]})\Kstarm}, \decay{\Bm}{(\decay{\Dstarz}{\Dz[\gamma]})\Kstarm} and \decay{\Bd}{(\decay{\Dstarp}{\Dz[\pip]})\Kstarm} are fixed separately for the 0 and $\pm$1 helicity states of the \Dstar. The expected yield ratios, $c^{Bu,\gamma}_0$, $c^{Bd,\pi}_0$, $c^{Bu,\gamma}_{\pm 1}$ and $c^{Bd,\pi}_{\pm 1}$, can be individually calculated from knowledge of the ratio of production of the relevant decays and the ratio of the efficiencies of the relevant decays passing the selection, detailed in Section \ref{sec:selection}, for example, 
\begin{align}
...
An extended maximum likelihood fit to the invariant B mass in the \kpi and \kpipipi favoured mode is performed using the shapes discussed in Sections \ref{sec:massfit:signal}, \ref{sec:massfit:combinatorial} and \ref{sec:massfit:partreco}. The total exetended likelihood PDF used to model the data is given by,
\begin{equation}
P_{tot}(x_i;\theta) = N_{sig}P_{sig} + N_{comb}P_{comb} + N_{dstkst}P_{dstkst} \text{ ,}
\label{totalpdf}
\end{equation}
where $x_i$ are the masses of the candidates and $\theta$ are the set of paramaters to be optimised, namely $\{\mu, \sigma, \beta, f_0\}$, with the expected yields of the signal, combinatorial and partially reconstructed yields, $N_{sig}$, $N_{comb}$ and $N_{dstkst}$, as additional freely varying parameters to be detemined, as described in Section \ref{sec:massfit:likelihood}. The signal PDF, $P_{sig}$, is given by Equation~\ref{DCBshape}, the partially reconstructed PDF, $P_{dstkst}$, is given by Equation~\ref{partrecofunction} and the combinatorial PDF, $P_{comb}$, is given by Equation~\ref{combpdf}, where the slope parameter $\beta$ is able to vary in the fit. The yield of the signal and combinatorial PDFs, $N_{sig}$ and $N_{comb}$, are left to vary without constraint, as well as the peak position, $\mu$, and width, $\sigma$, of the signal PDF. The only parameters able to vary in the partially reconstructed background are the yield ratio between the 0 and $\pm$1 amplitudes, $f_0$, and the overall yield, $N_{dstkst}$.
