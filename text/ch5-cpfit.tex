...
Section \ref{sec:backgrounds:charmless} shows that there is a possibility for residual charmless contribution to be present in \pipi mode. In order to estimate the systematic uncertainty associated with this possible charmless contribution, Method 2 is used by generating charmless events in the \pipi mode according to a Gaussian whose mean is the expected number of charmless events and the width is the corresponding uncertainty. This expected value is rounded to a whole number and randomly distributed between \Bp and \Bm as an additional contribution to the yield in the signal region. 
