...
The mass fit model described in Section \ref{sec:massfit} is used to model all \Dz modes in the \CP fit. However, there is an additional source of background from \decay{\Lb}{\Lc\Kstarm}, described in Section \ref{sec:backgrounds:Lb2LcKst}, which is included in the \kk model only. The shape used to model this \decay{\Lb}{\Lc\Kstarm} contribution is a Cruijff PDF, defined as,
\begin{equation}
...
\begin{equation}
\log\mathcal{L}_{tot} =  \sum_{m}\sum_{q}\sum_{t}\sum_{r}\left[\sum_{x_i} \log{\mathcal{L}}_{m,t,r}^q\left( \theta, N; x_i \right) \right]
\end{equation}
...
\caption{Results of the \CP fit for the two-body modes, summed over all \KS reconstruction types and data-taking periods after the fit is performed, for \Bp (left) and \Bm (right) decays.}
\label{results2body}
\end{figure}
...
\caption{Results of the \CP fit for the four-body modes, summed over all \KS reconstruction types and data-taking periods after the fit is performed, for \Bp (left) and \Bm (right) decays.}
\label{results4body}
\end{figure}
